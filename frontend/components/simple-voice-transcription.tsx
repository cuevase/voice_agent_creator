...
interface SimpleVoiceTranscriptionProps {
  companyId?: string;
}

const SimpleVoiceTranscription = ({ companyId }: SimpleVoiceTranscriptionProps) => {
  ...
  const startRecording = async () => {
    try {
      ...
      console.log('🔍 SimpleVoiceTranscription: Credit check:', {
        requiredCredits,
        hasCredits,
        service: 'voice_call'
      })
      ...
      console.log('🔍 SimpleVoiceTranscription: Auth Debug - User ID:', userId)
      console.log('🔍 SimpleVoiceTranscription: Company ID:', companyId)

      // Create WebSocket connection with user and company IDs
      const wsUrl = `http://localhost:8000/deepgram/websocket/test?company_id=${companyId}&user_id=${userId}`
      console.log('🔍 SimpleVoiceTranscription: WebSocket URL with parameters:', wsUrl)
      ...
      wsRef.current.onmessage = (event) => {
        try {
          ...
          if (data.type === 'status' && data.session_id && data.original_session_id) {
            console.log('🧪 Agent Lab - Received initial status with session details:', data);
  ...
  const stopRecording = () => {
    ...
    if (currentSessionId) {
      console.log('🧪 Agent Lab - Session ended:', currentSessionId);
    }
  ...
  const sendToAgent = async (userText: string) => {
    const currentSessionId = sessionIdRef.current || sessionId;
    console.log('🧪 Agent Lab - Sending to agent:', { 
      userText, 
    ...
    try {
      ...
      console.log('🧪 Agent Lab - Request payload:', {
      ...
      if (!response.ok) {
        ...
        console.error('🧪 Agent Lab - Backend error response:', {
      ...
      console.log('🧪 Agent Lab - Response received:', {
      ...
      if (data.audio) {
        console.log('🧪 Agent Lab - Playing audio response...');
        // Play the audio response
        const audio = new Audio(`data:audio/mpeg;base64,${data.audio}`);
        
        audio.onloadstart = () => console.log('🧪 Agent Lab - Audio loading started');
        audio.oncanplay = () => console.log('🧪 Agent Lab - Audio can play');
        audio.onplay = () => console.log('🧪 Agent Lab - Audio started playing');
        audio.onerror = (e) => console.error('🧪 Agent Lab - Audio error:', e);
        
        audio.play().catch(error => {
          console.error('🧪 Agent Lab - Failed to play audio:', error);
        });
        
        setStatus('Playing AI response...');
      } else {
        console.log('🧪 Agent Lab - No audio response received');
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold">🧪 Agent Lab - Live Mode</h3>
          <p className="text-gray-600">Test new agent architecture with real-time conversation</p>
        </div>
      </div>
...
export default SimpleVoiceTranscription; 